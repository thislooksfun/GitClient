...
class Auth2fViewController: PortraitViewController, UITextFieldDelegate
{
	...
	private func authCallback(state: Auth2fState)
	{
		let messageArea = self.nav.topViewController as! MessageAreaController
	...
	@IBAction func displayMethod(sender: UIButton) {
		...
		if nav.viewControllers.count == 1 {
			nav.topViewController?.performSegueWithIdentifier(self.authType == Auth2fType.App ? "ShowAppHelp" : "ShowSMSHelp", sender: nil)
		}
	}
	...
	func setAuthType(type: Auth2fType) {
		self.authType = type
	}
